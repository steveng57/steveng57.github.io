---
title: Munchies Tray
description: Hey hey, it's the munchies.
date: 2023-10-27 18:32:00 -0500
last_modified_at: 2024-02-17 18:32:00 -0500
category: [Woodworking, Home Decor]
tags:
  [Woodworking, Walnut, Trays, Epoxy, Inlay, Home Decor, Shaper Origin, Dominos]
image:
  path: IMG_0079.avif
  thumb: /thumbnails/IMG_0079.avif
  alt: Hey hey it's the munchies
media_subpath: /assets/img/posts/munchies-tray
---
## Origins

When I set out to do this, I hadn't really thought of the complications involved in making angled cuts like this. I mean, I knew there would be some, but I had to really dig deep and resurrect some old trigonometry skills. Nah, not really; I used an online calculator, but still, it involved a lot of weird angles. It's a 15&deg; angle to the sides, but when you are doing the cuts, it's a double mitre angle cut at a 13.1&deg; tilt and a 34&deg; mitre. Yikes.

> **Mitre Calculator**: For those who are interested in a really good online mitre cut calculator, here is the one I used: [Compound Angle Calculator]
{: .prompt-tip }

## Under Construction

{% include figure.html img="munchies-tray-sketchup.avif" align="center" %}
{% include clear-float.html %}

This is the {{ site.data.links.sketchup }} drawing used to plan the whole thing.

It ended up being bigger than this in the end, as I called an audible during the construction process. Doing such a reckless thing can often come back to haunt you, but in this case it worked out okay. The overall size was never an issue, it was all those angles that were the risky part.
{% include clear-float.html %}

{% include figure.html img="IMG_0074.avif" %}
{% include clear-float.html %}

As seems to be my usual for mitres these days, I used the {{ site.data.links.domino }} to add strength to the mitre and make it easy to glue up.  You have to use the really small 4 mm Dominos, but even then, you also have to be careful that you don't blow through the opposite side when cutting your Domino hole.  Because that looks downright messy and non-pretty.

{% include clear-float.html %}

{% include figure.html img="IMG_0079.avif" align="left" %}

### Crazy Angles

It was a fun build. While the angles were indeed odd, it was a lot of fun to see it come together.
{% include clear-float.html break=0 %}

## In Production

A tray full of munchies in its new home, which is normally resting upon the [extension table](/posts/extension-table) from the previous post.

{% include figure.html img="IMG_0643.avif" align="center-wide" %}

## Materials

- Walnut from {{ site.data.links.kjp }}
- Grey epoxy by {{ site.data.links.chillepoxy }}
- {{ site.data.links.shaper }} for the lettering
- {{ site.data.links.domino }} for mitre joinery
{: .sjg-list }

[Compound Angle Calculator]: https://jansson.us/jcompound.html
